...
module FConstrHash : Hashtbl.HashedType
  with type t = fconstr =
struct
  type t = fconstr
...
module FConstrMemo = Hashtbl.Make(FConstrHash)

type fconstr_ctx = fconstr FConstrMemo.t

let make_fconstr_ctx () = FConstrMemo.create 17

let copy_fconstr ?ctx (x : fconstr) : fconstr =
  (* note: it could be worthwhile to try to copy the [fconstr] lazily.
     doing this would probably mean that we should use a better
  *)
  let module SF = FConstrMemo in
  let memo = match ctx with None -> make_fconstr_ctx () | Some memo -> memo in
  ...
  and copy_fterm = function
    ...
    | FEvar (e, es, ts, xs) -> FEvar (e, es, copy_usubs ts, xs)
